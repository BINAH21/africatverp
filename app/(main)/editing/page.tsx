...
import {
  Edit, Video, CheckCircle, Plus, Trash2, Download, Archive, XCircle,
  Send, Search, Filter, User, Tag, Clock, CheckSquare, Square,
  FileDown, FolderArchive, Share2, Copy, Settings, Grid, List,
  ChevronDown, ChevronUp, Eye, Play, Pause, AlertCircle, FileText,
  Package, Code, Layers, FileCode, Wrench, Upload, MessageSquare,
  ArrowRight, ArrowLeft, Calendar, Star, Award, Users, FileCheck,
  FileX, CheckCircle2, X, Save, X as XIcon
} from 'lucide-react';
import { useVideoEditingStore, VideoEditingProject, EditingProgram } from '@/lib/video-editing-store';
...
export default function EditingPage() {
  ...
  const [editingProject, setEditingProject] = useState<VideoEditingProject | null>(null);
  ...
  const [formData, setFormData] = useState<Partial<VideoEditingProject>>({
    title: '',
    description: '',
    status: 'draft',
    assignedTo: '',
  ...
  const [programFormData, setProgramFormData] = useState<Partial<EditingProgram>>({
    name: '',
    version: '',
    description: '',
    category: '',
  ...
  const handleEdit = (project: VideoEditingProject) => {
    setFormData({
      ...project,
  ...
  const handleSave = () => {
    ...
    if (editingProject) {
      ...
      } as Partial<VideoEditingProject>);
      ...
      addProject({
        ...
        files: [],
      } as Omit<VideoEditingProject, 'id' | 'createdAt' | 'updatedAt' | 'workflow' | 'orders'>);
  ...
  const handleEditProgram = (program: EditingProgram) => {
  ...
  const handleSaveProgram = () => {
    ...
    if (editingProgram) {
      updateProgram(editingProgram.id, programFormData as Partial<EditingProgram>);
    } else {
      addProgram({
        ...programFormData,
        category: programFormData.category || 'General',
      } as Omit<EditingProgram, 'id' | 'createdAt' | 'updatedAt'>);
    }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Video Editing</h1>
          <p className="text-gray-600">Manage video editing projects and workflows</p>
        </div>
      ...
      {activeTab === 'projects' && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[
            { label: 'Active Projects', value: stats.active, icon: Edit, color: 'bg-purple-500' },
            { label: 'In Progress', value: stats.inProgress, icon: Video, color: 'bg-yellow-500' },
            { label: 'Completed', value: stats.completed, icon: CheckCircle, color: 'bg-green-500' },
            { label: 'Total Projects', value: stats.total, icon: FileText, color: 'bg-blue-500' },
          ].map((stat, index) => {
          ...
          {filteredProjects.length === 0 ? (
            ...
            filteredProjects.map((project, index) => (
              ...
              >
                ...
                <div className={`${viewMode === 'grid' ? 'h-48' : 'w-32 h-32 flex-shrink-0'} bg-gradient-to-br from-purple-400 to-purple-600 flex items-center justify-center relative`}>
              ...
              <div className="bg-gradient-to-r from-primary-500 to-primary-600 p-6 text-white">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-primary-100 text-sm">
                      {editingProgram ? 'Update program details' : 'Add a new editing program or software'}
                    </p>
                  </div>
                  <button
                    onClick={() => setShowProgramModal(false)}
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="e.g., Video Editor, Color Grading, Audio"
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                      />
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g., C:\Program Files\Adobe\Premiere Pro"
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                    />
                  </div>
